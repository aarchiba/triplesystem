...
if False:
    logger = logging.getLogger('threebody')
    logger.setLevel(logging.DEBUG)
    fh = logging.FileHandler("threebody.log")
...
class Fitter(object):
    ...
    def __init__(self, files=None, only_tels=None, tzrmjd_middle=False,
                 ...
                 fit_pm=False,
                 fit_px=False,
                 special=True,general=True,priors=[]):
        ...
        if not self.t2_astrometry:
            ...
            if fit_pos and 'd_RAJ' not in self.best_parameters:
                ...
                self.best_errors['d_RAJ'] = 1
                self.best_errors['d_DECJ'] = 1
            ...
            if fit_pm and 'd_PMRA' not in self.best_parameters:
                ...
                self.best_errors['d_PMRA'] = 1
                self.best_errors['d_PMDEC'] = 1
        ...
        if self.tzrmjd_base is None:
            del self.best_parameters['tzrmjd']
            #del self.best_parameters['f0']
            #del self.best_parameters['f1']
    ...
    def residuals(self, p):
        ...
        debug("Back from compute_orbit after time %s" % o['time'])
    ...
    def compute_linear_parts(self, p=None, t_psr=None):
        ...
        tzrmjd_s = scipy.optimize.brent(err,
            brack=(0,(self.mjds[-1]-self.base_mjd)*86400),
            tol=1e-160)
