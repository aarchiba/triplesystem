...
class MPIPool(object):
    ...
    def map(self, function, tasks):
        ...
        if (not self.loadbalance) or (ntask <= self.size):
            ...
            if self.debug:
                print("No load balancing; {0} tasks and {1} workers."
                      .format(ntask, self.size))
            ...
            if self.debug:
                print("Load balancing; {0} tasks and {1} workers."
                      .format(ntask, self.size))
            ...
            for i, task in enumerate(tasks[0:self.size]):
                ...
                self.comm.send(task, dest=worker, tag=i)
            ...
            for itask in range(ntask):
                ...
                if ntasks_dispatched < ntask:
                    ...
                    if self.debug:
                        print("Sent task {0} to worker {1} with tag {2}."
                              .format(task, worker, i))
                    # Send out the tasks synchronously; isend can barf if you don't wait.
                    self.comm.send(task, dest=worker, tag=i)
                    ntasks_dispatched += 1
